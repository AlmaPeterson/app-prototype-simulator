...
- **See:** Job name and a tappable status subtitle ("Active / No Bid Yet • Switch job"). Job context is deliberately sticky: workers pick a job once and it stays current until they open another. A **Prep for Tomorrow** section (QA note 2026-07-07) under the clock button lists training videos for this job's upcoming (not-yet-complete) scheduled tasks — your own assignments when you have any, otherwise the job's — each card showing its schedule date and a Watch or green Ready badge ("watched — no video needed on-site" once viewed today/yesterday). Hidden for supplier-role users or when no upcoming task has a training module.
- **Edit:** Nothing.
- **Do:** Tap a pending prep card → training-video in prep mode (watch tonight; the on-site task gate then skips the video for a day). Tap the subtitle (or tap the Home tab again while here) → jobs list to switch jobs. Clock In / Out (→ field-clock; hidden for supplier-role users). Quick actions: Schedule, Kits, View Bid or + Create Bid (label switches on whether the job has a bid), Time Sheet. "Full Job Details" → job-detail. ← Jobs.
...
- **Edit:** Everything writes straight to the DB on input:
  - Header: date, expires, bid #, project (bid title), customer name/phone/email (updates the customer record), lead craftsman, site address override. Bid # and expires are auto-filled (QA notes 2026-07-07): new bids get a generated `BID-<year>-<seq>` number and an expiration of bid date + 30 days at creation (openBid), and bids missing either are backfilled on open — expiration is a fixed price-validity window, deliberately not derived from job duration. Both stay editable.
  - Divisions: check/uncheck to include (insert / soft-delete a `bid_divisions` row).
  - Schedule: start, completion, procurement days, on-site days (overrides the auto-sum). Est. completion auto-calculates as start + procurement + on-site working days (Mon–Sat, Sundays skipped; on-site falls back to the division auto-sum when the override is blank) and re-derives whenever start/procurement/on-site or the division list changes; a hand-typed completion date holds until one of those changes.
...
- **Edit:** Labor rate ($/hr) and guys-on-site (drives `days = ceil(hours / (8 × guys))`); division scope (shown on the proposal); tasks — each with include-checkbox, category, name, scope, qty type (dropdown shows full names — Square Feet / Linear Feet / Count / Each / Hours — while the SF/LF/C/EA/HR codes are what's stored), qty, hrs/each; per-task materials — item, supplier, price, qty. Task hours round to the nearest quarter hour (15 min) in both the live calc and what's saved; the hrs/each spinner steps by 0.25, task qty and material qty by whole numbers (typed decimals still accepted), material price by cents. Add/remove tasks and materials freely.
...
- **Edit:** Nothing.
- **Do:** "Send to Customer" — generates and downloads a real jsPDF proposal (header, division costs with contingency, project schedule, 50/20/20/10 payment plan; that document is what the customer would receive) and moves the bid to `status: 'sent'` with `sent_at`. Already-sent bids show "Download PDF (sent {date})" instead (QA note 2026-07-07) — a pure re-download that leaves the sent status and `sent_at` untouched. ← Back to Bid Creation.
...
- **Do:** Tap the job chip → jobs list to switch jobs. Clock In — **requires a selected job** (alerts and routes to the jobs list otherwise); creates a `time_entries` row; clocking in while the simulated GPS is off-site records it as `flagged` for manager review. Record activity: Driving / Lunch / Task / Material (opens the material log). **Task** (requires being clocked in) opens the task picker sheet — the company's task modules, each with your competency badge (Student/Exposure/Competent/Mastery — defaults to Student if never evaluated) and a subtitle explaining what that level allows; high-hazard tasks are flagged "PPE required". Picking one runs the gate chain (training-video → co-sign/PPE below) — **unless its training video was already watched today or yesterday** (via job-home's Prep for Tomorrow, QA note 2026-07-07), in which case the video step is skipped straight to the competency branch; once cleared, the task starts recording in the activity log and opens a `phase_logs` row against the running time entry — tapping End Task stamps its `ended_at`. Tapping a Pending training card also routes into the Task gate chain. Clock Out — the button is visibly disabled (dimmed) until all three checklist items are done, and clocking out ends any ongoing task. "Daily Scorecard & Timesheet" — end-of-day is scorecard-first: opens your self-assessment, which returns to review-time for submission (skips straight to review-time if today's scorecard is already in).

### training-video.html
- **See:** Two modes on the same player. **Gate mode** (opened from the field-clock task picker): "{Task} — Safety Training", subtitle notes it can't be skipped. **Prep mode** (opened from job-home's Prep for Tomorrow, QA note 2026-07-07): "{Task} — Training", subtitle explains watching now clears tomorrow's on-site video. Both: a simulated 3-second video progress bar, a Continue button that stays disabled until the bar finishes.
- **Edit:** Nothing.
- **Do:** Continue (after 3s) — **Prep mode:** records the training completion (same `training_assignments` trail the gate uses) and returns to job-home with a confirmation toast; no competency gates involved. **Gate mode:** records the training completion, then routes by competency: Mastery → PPE gate (if high-hazard) or back to the field clock (task starts recording); Competent → co-sign modal (a master's name confirms, then continues); Student/Exposure → blocked with a message, back to the field clock.
...
- **See:** A Gantt-style task grid — tasks down the left (sticky column), workdays across the top (**Monday–Friday only; Saturday and Sunday are locked days of rest and never show**), a colored block under each day a task runs. Two ranges: **Week** (one work week, prev/next arrows, anchored to the seeded cluster week of Jun 29 2026, always fits the page) and **Full** (every scheduled weekday from earliest to latest event on file; fixed-width columns that scroll sideways with edge-fade hints). Scoped to the sticky current job when one is set (tappable subtitle → jobs list); otherwise all jobs' events show. Everyone signed in sees `view_type: 'worker'` events (customers get that information via the bid PDF, not an app surface).
- **Auto-generated from the bid (QA note 2026-07-13):** opening the page for a job with a bid and no bid-derived rows yet derives the schedule from the bid sheets via `generateScheduleFromBid`: each included labor task becomes one `schedule_events` row lasting `ceil(hours ÷ (8 × guys))` workdays, tasks run back-to-back in proposal order, starting on the bid's start date (fallback: the job's scheduled start, weekend-snapped forward), colored per division. Rows are best-effort matched to a `task_modules` row by name so the JIT training gate still fires. The view jumps to Full so the whole run is visible.
- **Do:** **Sync from Bid** (shown only when the current job has a bid) — re-derives after bid edits, soft-deleting prior bid-derived rows but leaving manual entries alone. "+ Add" bottom sheet (title, optional job, start/end dates defaulting to the viewed Monday) — inserts a manual row (`bid_division_id: null`, assigned to you, blue, `status: 'scheduled'`). Toggle Week/Full, shift weeks (Week only), tap a task's name cell (rows with a job) → that job.
